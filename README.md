...
- **Auto-Stop Timer**: Set a specific duration after which the mocking service will automatically stop.
- **Auto-Start on Launch**: Automatically resume mocking with your last saved location when the app is opened.
- **Persistent Notification**: A non-dismissible foreground service notification that tracks status and displays the remaining time for auto-stop.
- **System Integration**: One-click access to Developer Options (Mock App Selection) and Battery Optimization settings.
...
### 功能特點
- **虛擬定位**：輕鬆將裝置定位修改至任何指定座標。
- **隨機漂移**：每 2 秒微調位置與高度，模擬真實的人類移動行為。
- **高度支援**：支援手動輸入高度，或從目前位置獲取真實高度。
- **Google Play 服務整合**：可選擇透過 Fused Location Provider 獲取更穩定、準確的定位更新。
- **自動停止計時器**：設定特定時間，時間到後自動停止模擬服務。
- **啟動時自動開始**：開啟 App 時自動以最後一次儲存的位置開始模擬。
- **常駐通知欄**：具備「划不掉」特性的前景服務通知，可即時查看狀態並顯示自動停止的剩餘時間。
- **系統整合**：提供一鍵跳轉開發人員選項（選取模擬位置應用程式）及電池最佳化設定。
